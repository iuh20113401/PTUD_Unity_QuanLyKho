...
class SanPham{
    ...
    function layChiTietSanPham($maSanPham){
        $stmt = $this->conn->prepare("SELECT ctsp.MaChiTietSanPham, ctsp.SoLuongTon, sp.MaSanPham, sp.TenSanPham, ctsp.NgaySanXuat, ctsp.NgayHetHan, ctsp.SoLuongChoXuat, ctsp.DonVi, k.TenKho FROM sanpham as sp join chitietsanpham as ctsp on ctsp.masanpham = sp.masanpham JOIN Kho as k on k.MaKho = ctsp.MaKho where ctsp.maSanPham = ?");
